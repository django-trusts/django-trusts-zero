"""Zero-owned AppConfig and canonical backend against core 1.0.0.dev3."""
...
class CanonicalBackendIdentityTests(SimpleTestCase):
    def test_zero_backend_is_the_canonical_historical_class(self):
        ...
        with self.assertRaises(ImportError):
            from trusts.backends import TrustModelBackend as CoreHistorical  # noqa: F401
...
class OwnerPresentNeverCallsKernelTests(SimpleTestCase):
    def test_core_has_no_kernel_config(self):
        self.assertFalse(hasattr(trusts_apps, 'kernel_config'))
        self.assertFalse(hasattr(trusts_apps, 'AppConfig'))

    def test_zero_config_ready_resolves_without_kernel_config(self):
        owner = zero_config()
        owner.ready()
        self.assertFalse(hasattr(trusts_apps, 'kernel_config'))

    def test_mixin_owner_present_resolves_without_kernel_config(self):
    ...
    def test_zero_config_helper_resolves_without_kernel_config(self):
...
class AuthorizationThroughZeroBackendTests(TestCase):
    ...
    def test_has_perm_and_permitted_use_zero_owner(self):
